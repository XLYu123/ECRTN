...
data_y = np.array([1] * 30+[2] * 30+[3] * 30+[4] * 30+[5] * 30)
# print(data_y.shape)
y_B = one_hot_MPT(data_y, depth=7)


# print('--Target Domain--')
print('Feature shape:', x_B.shape, 'Label shape', y_B.shape, )

print('Label number are:', y_B[0].numpy(), 'for class 0 (normal)', y_B[30].numpy(), 'for class 0 (normal)',
      y_B[100].numpy(), 'for class 0 (normal)')

# load test data
faults2 = [ 'C0','C1','C2','C3','C4','C5']
data_x_T = []
data_y_T = []
for fault2 in faults2:
    x, _ = LoadData_pickle_T(path=,
                             name=fault2)
    data_x_T.extend(x)

data_x_T = np.array(data_x_T)
data_y_T = np.array([0] * 300+[1] * 300+[2] * 300+[3] * 300+[4] * 300+[5] * 300)
...
class CNN(keras.Model):
    ...
    def _get_1dcnn(self):
        ...
        # (None, 128)
        enc = Flatten()(enc)
        logits = keras.layers.Dense(7, activation='softmax')(enc)
    ...
    def train(self, train_db, test_db, epochs):
        ...
        np.savetxt ('./results/cnn_resnet_eca/2res_eca_11.txt',af)
        np.savetxt('./results/cnn_resnet_eca/2res_eca_loss_11.txt',los)
